...
- Document Explorer: synchronous parser work (`extract_text`, `get_page_count` — pdfplumber / openpyxl / python-docx / python-pptx) now runs off the event loop via `asyncio.to_thread`. Previously a folder upload of hundreds of files held the loop for the duration of every parse, starving websocket pings, RLM streams, document-list polls, and any other concurrent request on the same process
...
- Document Explorer: extracted text is now capped at 16 MiB per file with an inline truncation marker, defending against decompression-bomb DoS through `.xlsx` / `.pptx` / `.docx` / `.pdf` (a heavily-compressed file under the 50 MB upload cap can unpack to hundreds of MB of plain text). The cap fires at the public `extract_text` boundary so every format inherits it, mirroring the existing `truncate_code_output` pattern in `rlm/prompts.py`. Bounds in-process memory and the on-disk document store, and reduces attacker-controlled bytes flowing into the RLM context. The cap is enforced in UTF-8 bytes (not characters), so non-ASCII content is bounded at the same on-disk size as ASCII content — a previous character-based check could have allowed up to ~64 MiB of multi-byte content (e.g. CJK text) past the 16 MiB ceiling
