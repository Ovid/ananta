...
- All explorers: `BaseTopicManager.rename` now also rejects targets whose slug matches a corrupt-meta sibling's directory. The previous check only inspected readable display names — if a sibling topic.json was corrupt and its directory squatted on the rename target's slug, the rename succeeded silently. If the corrupt sibling were later repaired (e.g. via `create()` during a subsequent upload), the system would end up with two topics sharing the same display name (S17)
- Document Explorer: file-type icons in the document-list sidebar now resolve correctly for content types with parameter suffixes (e.g., `text/plain; charset=utf-8`). Browsers may include parameters and the backend stores `UploadFile.content_type` verbatim, so the previous exact-match table missed legitimate entries and showed the generic folder icon instead (S37)
- Document Explorer: `folder-walk.readAllEntries` no longer hangs forever when `reader.readEntries` synchronously throws on a recursive batch. The recursive call is scheduled via `queueMicrotask`, which means a sync throw is not caught by the outer Promise constructor — it became an unhandled rejection and `walkEntries` never resolved or rejected, stranding the upload modal in preflight. The recursive body now wraps the call in try/catch and surfaces the throw via `reject` (S31)
- Document Explorer: a persistent failure of `/api/documents` or `/api/documents/uncategorized` no longer leaves the sidebar empty with zero diagnostic feedback. The two `.catch(() => {})` arms in `App.tsx` now log via `console.warn` so the failure is visible to anyone debugging (S34)
- Document Explorer: the folder-upload summary's `upload` failure row now shows a clean reason ("files too large for this request", "server error (500)", etc.) instead of the doubled-up "upload failed: …" prefix that read awkwardly alongside the row's own `upload` label (S30)
- Document Explorer: a folder upload that errored AT THE SAME TIME as the user pressed Cancel previously had its error silently swallowed — the user saw a clean cancel without learning the upload had also failed. The hook now logs the error via `console.warn` so it is at least visible to anyone debugging (S25)
- Document Explorer: the document context-menu's "Add to…" submenu no longer hides a topic just because that topic already contains a doc with the same filename. With folder uploads, distinct documents legitimately share filenames (e.g., several `README.md` / `__init__.py` / `index.ts` files in different subfolders, each a separate `project_id`). The previous label-equality eligibility clause silently dropped the duplicates from the submenu — they had no path to that topic from the UI. Eligibility is now strictly by `project_id` (I5)
- Document Explorer: single-file picker uploads (the click-pick / shift-select path through `App.tsx` → `api.documents.upload`) now render failures through the same friendly error formatter as the folder-upload path. Previously a 422 with a FastAPI auto-validation array detail rendered as `"[object Object],[object Object]"` in a toast, and a 413 surfaced as the bare `"Payload Too Large"` statusText. The single-file path now maps array details to a friendly summary, 413 to `"files too large for this request"`, and 5xx to `"server error (<status>)"` — matching the folder-upload UX (I4)
- Document Explorer: the upload form no longer attaches `relative_path` form entries when no file in the batch carries a real `webkitRelativePath`. Some webview / non-browser test environments expose `webkitRelativePath` as `undefined` rather than the empty string; `FormData.append` stringifies `undefined` to the literal string `"undefined"`, which then passed server-side validation and persisted as a real (bogus) `relative_path`. Missing values are now coalesced to the empty string when at least one file does carry a path (length-match preserved), and the field is omitted entirely when no file does (U1)
- Shared explorer factory: `PATCH /api/topics/{name}` and `DELETE /api/topics/{name}` (via `create_shared_router`) now map topic-manager `ValueError` to the same status taxonomy as the sibling `create_item_router` — duplicate-name conflicts surface as 409 and validation failures as 422 instead of every error leaking as 404. The two routers serve the same logical operation; their contracts now agree (S41)
...
- Document Explorer: folder-upload error messages prefer the server's FastAPI `{detail}` body when present, and otherwise map common status codes to plain-English text ("files too large for this request" instead of "upload failed: 413")
...
- Document Explorer: dropping a second folder while the first one is still being walked (large folder, slow disk) is now a no-op; the previous guard only fired after the user pressed Continue on the first folder, so a fast double-drop could race the two preflights
- Document Explorer: the folder-upload progress modal's "batch K of N" label now advances at the START of each batch instead of after each batch completes. Previously the modal trailed one batch behind reality for batches 2+ — during batch K (K>=2) the user saw "batch K-1 of N" until that batch finished. The hook's initial state covered batch 1, so batch 1 displayed correctly, but the bug was visible for any multi-batch upload
- Document Explorer: a folder-walk failure (browser permission denial, Safari `FileSystemEntry` quirk, DOMException, etc.) now surfaces as the upload summary modal with the failure recorded, instead of becoming an unhandled promise rejection that opened no modal and gave the user zero feedback. Previously only the hard 500-file cap-exceeded path surfaced as a summary; any other top-level rejection silently no-opped
- Document Explorer: the entire per-file disk-and-storage block of the upload route (mkdir-with-retry, write_bytes of up to 50 MiB, extract_text, get_page_count, meta.json write, create_project, store_document of up to 16 MiB extracted text, topic_mgr.add_item) now runs off the event loop in a single worker thread per file. Previously only the synchronous parser work (extract_text, get_page_count) was wrapped, so a folder upload of hundreds of files still held the loop for the remaining ~6 sync ops per file — starving websocket pings, RLM streams, document-list polls, and any other concurrent request on the same process
- Document Explorer: `DELETE /api/documents/{doc_id}` returns 404 for unknown ids instead of a confident "deleted" 200, matching the existing 404 from `GET` and `PATCH` on the same path
- Shared explorer factory: middleware ordering corrected. Starlette's `add_middleware` inserts at index 0, so the LAST add_middleware call wraps the OUTERMOST runtime layer — the previous order (BodySizeLimit added first, CORS last) actually produced a runtime stack with CORS outermost and BodySizeLimit innermost, contradicting the comment that claimed BodySizeLimit ran first. Reordered the calls so BodySizeLimit is added last and sits outermost as intended; future maintainers adding a body-reading middleware (request logging, metrics, audit) now have a defensive boundary they can reason about. Today's behaviour is unchanged because neither CORS nor the SameOrigin guard reads the body
...
- Document Explorer: cancelling upload A then dropping folder B no longer silently breaks B's Continue button. Previously, A's in-flight batch would settle after B's preflight installed `pending = B`, and A's aborted-branch unconditionally cleared `pending`, wiping B's payload — so pressing Continue early-returned and the modal sat stuck on preflight. The post-await clear is now guarded by the same identity check the cancel-vs-newer-controller fix used
...
- Document Explorer: `relative_path` form input is now validated server-side. The validator rejects: empty values, anything over 512 characters, control bytes (0x00–0x1f, 0x7f), backslashes, a leading `/`, and any path segment exactly equal to `..`. Common filename punctuation (apostrophes, parens, ampersands, plus signs, etc.) and non-ASCII characters are accepted. Previously the value was persisted verbatim, allowing disk-fill DoS via giant strings and a path-traversal / prompt-injection surface. The array length is also required to match `files` length
...
- Shared explorer factory now applies the same-origin guard to WebSocket handshakes (`/api/ws`) as well as mutating HTTP requests. Previously the guard short-circuited on `scope["type"] != "http"` and let any cross-origin browser open the WebSocket: a drive-by page could submit `query` payloads that drained the operator's LLM credits, triggered sandbox container starts, or exfiltrated streamed answers/traces over the same socket. The middleware now sends a 1008 close before `accept` when Origin/Host disagree on a WS connect
...
- All explorers: topic names are now stripped of surrounding whitespace at every entry point (`create`, `rename`, `_resolve`, ...). Previously `"Reports "` and `"Reports"` slugified to the same directory but stored different display names, so an upload with the trailing-space form created a topic that subsequent calls with the trimmed form could not resolve — surfacing as confusing 422 conflicts
- All explorers: `BaseTopicManager.rename` now rejects empty / whitespace-only new names with a clean 422. Previously the strip-then-validate path skipped this check (only `create` rejected via the empty-slug guard downstream), so `rename("Reports", "   ")` wrote `meta["name"] = ""` — leaving the topic on disk behind a label-less, unselectable sidebar row (soft data corruption). The empty-after-strip guard now lives in `_validate_name` so every entry point inherits it
...
- Document Explorer: extracted text is now capped at 16 MiB per file with an inline truncation marker, defending against decompression-bomb DoS through `.xlsx` / `.pptx` / `.docx` / `.pdf` (a heavily-compressed file under the 50 MB upload cap can unpack to hundreds of MB of plain text). The cap is enforced in UTF-8 bytes (not characters), so non-ASCII content is bounded at the same on-disk size as ASCII content. Per-format extractors stream-and-bail with a cumulative-bytes guard inside the per-page / per-paragraph / per-slide / per-row loop, so peak in-process memory is bounded too — not just the on-disk store and the RLM-context payload
- Document Explorer: a folder upload that auto-creates a topic and then has every per-file persist fail no longer leaves a label-less stub topic behind in the sidebar. The route now tracks whether the topic was new for this request and rolls it back on full failure. Pre-existing topics (created explicitly via `POST /api/topics`, or already carrying items from prior uploads) are left untouched (S6)
- Document Explorer: `_extract_pptx` now streams text frames inside each slide and bails out mid-slide once the cumulative cap is reached. Same class as the xlsx fix below — a slide with thousands of text frames previously accumulated all of them before the per-slide cap check ran (S3)
- Document Explorer: `.rtf` files are now read as CP1252 instead of UTF-8. RTF files commonly contain raw 8-bit bytes (Windows-1252) in legacy Word output. Reading as UTF-8 with `errors="replace"` substituted U+FFFD for every non-ASCII byte before striprtf could decode it — silent data loss on legitimate non-ASCII RTF content (S1)
- Document Explorer: `_extract_xlsx` now streams cells one at a time inside each row and bails out the moment the cumulative cap is reached. The previous list-comprehension materialised every cell of the row before any cap check could fire, so a maliciously crafted single-row workbook with millions of populated cells could allocate hundreds of MB of Python strings before `_truncate_to_cap` ran. The 50 MiB upload cap doesn't bound the in-memory expansion: openpyxl's `read_only=True` iterates lazily by row, but the per-row list comprehension realised all cells at once (I6)
- Document Explorer: `DELETE /api/documents/{id}` no longer leaves orphan projects in `state.ananta.list_projects()` when downstream cleanup (topic refs, upload directory) fails. Source-of-truth deletion of the Ananta project now runs FIRST; topic and filesystem cleanup are best-effort with logged failures. Previously the order was topics → filesystem → ananta-delete, so any error in the third step left the project alive in `list_projects()` indefinitely with the user-facing list endpoints filtering it out (no `meta.json`) and a retry returning 404 because the upload dir was gone — a silent permanent orphan
- RLM engine: document filenames in the per-query context-metadata assistant message are now wrapped with the same per-query untrusted-content boundary that already protects REPL output and document content. Filenames are user-controlled (uploaded `UploadFile.filename` is persisted verbatim) and previously rendered into the highest-trust position in the prompt without any boundary marker. A hostile filename could inject content that the model would treat as trusted instructions; with the wrap, the model is told to treat the filename channel as untrusted data
- Document Explorer: filenames are now strictly validated at upload AND rename time (I6). The validator rejects empty values, anything over 512 chars, control bytes (0x00–0x1f, 0x7f), `/`, `\`, and a bare `..`. Common filename punctuation (apostrophes, parens, ampersands, brackets, plus signs, hash, non-ASCII) is still accepted. Previously `DocumentRename.new_name` had no length cap or content check, so a single PATCH could write a multi-MB string into `meta.json` (ballooning every list-documents call) or land an injection-grade filename that surfaced in the per-query LLM context
- Document Explorer: filename and content_type fields rendered into the per-query LLM context (`_build_doc_context`) are now scrubbed for control bytes / newlines / `---` runs (I5). The C1 fix wrapped the `doc_names` channel against filename-driven prompt injection but missed this second channel, which appended `--- Document: {filename} (type: {content_type}) ---` directly to the user-role message without a boundary marker. The upload + rename validators (above) prevent NEW data from carrying injection payloads; this scrubbing closes the legacy-data gap (any meta.json written before validation existed remains on disk indefinitely)
- All explorers: `BaseTopicManager` mutating methods (`create`, `rename`, `delete`, `add_item`, `remove_item`, `remove_item_from_all`, `reorder_items`) now hold a per-instance lock across the read-modify-write cycle on `topic.json`. Without the lock, two thread-pool workers servicing concurrent uploads to the same topic could interleave the load + mutate + store cycle and silently lose document references — the documents themselves still landed on disk and surfaced under "Uncategorized", but never appeared in the topic. Soft data corruption that was hard to debug. The recent move of `_persist_one_upload` into `asyncio.to_thread` for folder uploads made the race meaningfully reachable for parallel uploads to the same topic (I2)
