...
class TestAddAndListRepos:
    ...
    def test_add_item_to_topic(self, tmp_path: Path) -> None:
        """Adding a repo to a topic makes it appear in list_items."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Frontend")
        mgr.add_item("Frontend", "project-abc-123")
        repos = mgr.list_items("Frontend")
    ...
    def test_add_multiple_repos(self, tmp_path: Path) -> None:
        ...
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Backend")
        mgr.add_item("Backend", "repo-1")
        mgr.add_item("Backend", "repo-2")
        mgr.add_item("Backend", "repo-3")
        repos = mgr.list_items("Backend")
    ...
    def test_add_item_to_nonexistent_topic_raises(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(ValueError, match="Topic not found"):
            mgr.add_item("Nonexistent", "repo-1")

    def test_list_items_nonexistent_topic_raises(self, tmp_path: Path) -> None:
        """Listing repos for a nonexistent topic raises ValueError."""
        mgr = CodeExplorerTopicManager(tmp_path)
        with pytest.raises(ValueError, match="Topic not found"):
            mgr.list_items("Nonexistent")
...
class TestRemoveRepo:
    ...
    def test_remove_item_from_topic(self, tmp_path: Path) -> None:
        """Removing a repo from a topic removes it from list_items."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Frontend")
        mgr.add_item("Frontend", "repo-1")
        mgr.add_item("Frontend", "repo-2")
        mgr.remove_item("Frontend", "repo-1")
        assert mgr.list_items("Frontend") == ["repo-2"]
    ...
    def test_remove_nonexistent_repo_raises(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(ValueError, match="Item not found"):
            mgr.remove_item("Frontend", "nonexistent-repo")

    def test_remove_item_from_nonexistent_topic_raises(self, tmp_path: Path) -> None:
        """Removing a repo from a nonexistent topic raises ValueError."""
        mgr = CodeExplorerTopicManager(tmp_path)
        with pytest.raises(ValueError, match="Topic not found"):
            mgr.remove_item("Nonexistent", "repo-1")
...
class TestSameRepoMultipleTopics:
    ...
    def test_same_repo_in_multiple_topics(self, tmp_path: Path) -> None:
        ...
        mgr.create("Frontend")
        mgr.create("Backend")
        mgr.add_item("Frontend", "shared-lib")
        mgr.add_item("Backend", "shared-lib")
        assert "shared-lib" in mgr.list_items("Frontend")
        assert "shared-lib" in mgr.list_items("Backend")
...
class TestListAllRepos:
    ...
    def test_list_all_items(self, tmp_path: Path) -> None:
        """list_all_items returns unique repos across all topics."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Frontend")
        mgr.create("Backend")
        mgr.add_item("Frontend", "repo-1")
        ...
        mgr.add_item("Backend", "repo-2")
        mgr.add_item("Backend", "repo-3")
        all_repos = mgr.list_all_items()
        assert sorted(all_repos) == ["repo-1", "repo-2", "repo-3"]

    def test_list_all_items_empty(self, tmp_path: Path) -> None:
        """list_all_items with no topics returns empty list."""
        mgr = CodeExplorerTopicManager(tmp_path)
        assert mgr.list_all_items() == []

    def test_list_all_items_no_duplicates(self, tmp_path: Path) -> None:
        """list_all_items does not return duplicates even if repo is in multiple topics."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("A")
        mgr.create("B")
        mgr.create("C")
...
class TestUncategorizedRepos:
    ...
    def test_list_uncategorized(self, tmp_path: Path) -> None:
        """list_uncategorized returns repos not in any topic."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Frontend")
        mgr.add_item("Frontend", "repo-1")
        all_known = ["repo-1", "repo-2", "repo-3"]
        uncategorized = mgr.list_uncategorized(all_known)
        assert sorted(uncategorized) == ["repo-2", "repo-3"]

    def test_list_uncategorized_all_categorized(self, tmp_path: Path) -> None:
        """When all repos are in topics, uncategorized returns empty list."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("All")
        mgr.add_item("All", "repo-1")
        mgr.add_item("All", "repo-2")
        uncategorized = mgr.list_uncategorized(["repo-1", "repo-2"])
        assert uncategorized == []

    def test_list_uncategorized_none_categorized(self, tmp_path: Path) -> None:
...
class TestFindTopicsForRepo:
    ...
    def test_find_topics_for_item(self, tmp_path: Path) -> None:
        """find_topics_for_item returns topics containing the repo."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Frontend")
        mgr.create("Backend")
        mgr.create("Empty")
        mgr.add_item("Frontend", "shared-lib")
        mgr.add_item("Backend", "shared-lib")
        topics = mgr.find_topics_for_item("shared-lib")
        assert sorted(topics) == ["Backend", "Frontend"]

    def test_find_topics_for_item_not_found(self, tmp_path: Path) -> None:
        """find_topics_for_item returns empty list if repo is not in any topic."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Frontend")
        assert mgr.find_topics_for_item("unknown-repo") == []
...
class TestRemoveRepoFromAll:
    ...
    def test_remove_item_from_all(self, tmp_path: Path) -> None:
        """remove_item_from_all removes the repo from every topic."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Frontend")
        mgr.create("Backend")
        mgr.add_item("Frontend", "shared-lib")
        mgr.add_item("Backend", "shared-lib")
        mgr.add_item("Backend", "other-lib")
        mgr.remove_item_from_all("shared-lib")
        assert mgr.list_items("Frontend") == []
        assert mgr.list_items("Backend") == ["other-lib"]

    def test_remove_item_from_all_not_in_any(self, tmp_path: Path) -> None:
        """remove_item_from_all does not raise if repo is not in any topic."""
        mgr = CodeExplorerTopicManager(tmp_path)
        mgr.create("Frontend")
        # Should not raise
        mgr.remove_item_from_all("nonexistent-repo")
...
class TestListTopicsNoDuplicates:
    ...
    def test_no_duplicate_display_names(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "dir-a" / "topic.json").write_text(
            json.dumps({"name": "RLMs", "items": ["repo-1"]})
        )
        (tmp_path / "dir-b").mkdir()
        (tmp_path / "dir-b" / "topic.json").write_text(
            json.dumps({"name": "RLMs", "items": ["repo-2"]})
        )
        topics = mgr.list_topics()
        assert topics == ["RLMs"]
