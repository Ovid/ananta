"""Tests for shared persistent web conversation session."""
...
def test_add_exchange_stores_document_ids(session: WebConversationSession) -> None:
    ...
    assert exchange["document_ids"] == ["doc-a", "doc-c"]

    # Verify persistence
    reloaded = WebConversationSession(session._file.parent)
    assert reloaded.list_exchanges()[0]["document_ids"] == ["doc-a", "doc-c"]
...
def test_corrupt_json_loads_as_empty(session_dir: Path) -> None:
    ...
    session_dir.mkdir(parents=True, exist_ok=True)
    (session_dir / "conversation.json").write_text("{bad json")
...
def test_save_is_atomic_on_crash(session_dir: Path) -> None:
    ...
    tmp_files = list(session_dir.glob("conversation.json.*"))
    assert tmp_files == []


def test_add_exchange_document_ids_defaults_to_none(
